...
let backgroundImage, spaceshipImage, bulletImage, enemyImage, gameOverImage, enemyBoss1Image, enemyBoss2Image
    , enemyBoss3Image;
...
function generateRandomValue(min,max){ // min,max = 최대값 최소값 미리 정해준것 / 화면 밖에서 생성되면 안되므로
...
function checkSpaceshipCollision(enemy) {
    return spaceshipX < enemy.x + 48 && spaceshipX + 60 > enemy.x && 
           spaceshipY < enemy.y + 48 && spaceshipY + 60 > enemy.y;
}
...
function update(){
    ...
    for (let i = 0; i < enemyList.length; i++) {
        if (enemyList[i] instanceof Boss1 && enemyList[i].alive) {
            ...
            if (currentTime - enemyList[i].lastSpawnTime > enemyList[i].enemySpawnInterval) {
                let e = new Enemy();
